...
$SQL['pass']="";//pass
$SQL['base']="aidir";//nom de la base de données
?><!DOCTYPE HTML>
<html>
    <head>
    ...
    <body>
        ...
        <a href="register.php">Retour accueil</a>
        
        <?php
        if(isset($_GET['code'],$_GET['email'])){
            //un lien est cliqué dans un mail, on recherche si le code et le email correspondent à une ligne dans la table "recup_mdp"
            ...
            if(!$mysqli) {
                ...
                $req=mysqli_query($mysqli,"SELECT * FROM recup_mdp WHERE code='$Code' AND email='$Mail'");
                if(mysqli_num_rows($req)==1){
                    ...
                    mysqli_query($mysqli,"UPDATE users SET mdp='".md5($NouveauPass)."' WHERE email='$Mail'");
                    ...
                    mysqli_query($mysqli,"DELETE FROM recup_mdp WHERE code='$Code' AND email='$Mail'");
                    echo "Votre nouveau pass temporaire vient d'être envoyé par email.";
                } else {
                    echo "Lien incorrect.";
                }
            }
            ...
            if(isset($_POST['valider'])){
                ...
                if(empty($_POST['email'])){
                    echo "Le champs email n'est pas renseigné.";
                } else {
                    ...
                    if(!$mysqli) {
                        ...
                        if(mysqli_num_rows($req)!=1){
                            ...
                            mysqli_query($mysqli,"INSERT INTO recup_mdp SET code='$Code', email='$Mail'");
                            $Lien=$_SERVER['HTTP_HOST']."/mot-de-passe-oublie.php?code=$Code&email=$Mail";
